...
```
[GPT Image - repaint frame untuk DELIA (video VRM 3D -> karakter anime 2D)]
Input: (1) frame dari klip denise_01 (repaint_src_*.png di input/denise_01/repaint/), (2) sheet Delia (delia_denise_pose_01/delia_ref_sheet_01.png)
Simpan hasil: E:\AI\ComfyUI\input\denise_01\repaint\delia_repaint_5p8s.png (resolusi bebas; makin tinggi makin bagus)
(opsional kandidat kedua: repaint_src_11p5s_1080.png -> simpan delia_repaint_11p5s.png)
```
...
- Frame terpilih `repaint_src_5p8s` (tangan ke atas, kaki telanjang terlihat penuh) - mematikan halusinasi stocking/boot di seluruh klip; tangan sumber = glove hitam, wajib jadi kulit.
- Frame `repaint_src_11p5s` (jari terbuka) = kandidat kedua kalau tangan masih ghost.

## Konfigurasi workflow

| Param | Value |
|---|---|
| Driving video | `denise_01/denise_01.mp4` (360x640, 30fps, 12,7s -> 3 window @124f + anchor 5f) |
| Resolution | 0.6 MP |
| Sigma | `euler` + 8 titik CustomSigmas (7 update) - config proven denise run 00005 @0,6 MP |
...
- Workflow: `eikei-plan/custom/workflows/mv_15s/ongoing/h3_r2v_viggle_delia_01_chunked_euler4.json`
